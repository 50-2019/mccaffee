...
namespace Caffee
{
    public partial class CreateWaiterAccount : Form
    {
        ...
        private void buttonCreateOwnerAcc_Click(object sender, EventArgs e)
        {
            ...
            {
                InfoWarningError infoWarning = new InfoWarningError("Your password must contain an uppercase letter, lowercase letter, a digit and a special character!", "error", "OK", "CANCEL");
                infoWarning.ShowDialog();
                check = 0;
            }
            ...
            if (check == 3)
            {
                ...
                {
                    InfoWarningError infoWarning = new InfoWarningError("Username already exists!\nTry another one!", "error", "OK", "CANCEL");
                    resetTextBoxes();
                    infoWarning.ShowDialog();
                    check = 0;
                }
